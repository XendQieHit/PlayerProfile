...
public class Status {
    ...
    private final ArrayList<SubStatus> subStatusArrayList = new ArrayList<>();

    public Status(Player player) {
        this.player = player;
        idle = new Idle(player);
        afk = new AFK(player);
        this.subStatusArrayList.add(afk);
        this.subStatusArrayList.add(idle);
    }

    public ArrayList<SubStatus> getAllSubStatuses() {
        return subStatusArrayList;
    }
    ...
    public void loadSetting() {
        for (SubStatus subStatus : subStatusArrayList) {
    ...
    public List<String> getAllSubStatusNames() {
        ...
        for (SubStatus subStatus : this.getAllSubStatuses()) {
            list.add(subStatus.getClass().getSimpleName().toLowerCase());
        }
        return list;
    }
}
